...
const createLoginWindow = (base_url: string): void => {
  ...
  const handler = (details: Electron.OnCompletedListenerDetails) => {
    ...
    if (details.url.includes(`https://${base_url}/login/authenticate`)) {
      ...
      if (details.statusCode === 200 && details.responseHeaders) {
        ...
        console.log('Token is: ', token, 'create render window')
        createWindow()
        login_window.close()
      }
    }
  }
  login_session.webRequest.onCompleted({ urls: [`https://${base_url}/*`] }, handler);
  ...
  login_window.loadURL(`https://${base_url}/login`)
}
...
const createWindow = async (): Promise<void> => {
  if (!token) {
    return createLoginWindow(base_url)
  }
  ...
  main_session.webRequest.onBeforeSendHeaders({
    urls: [`https://${base_url}/*`, `http://*.${base_url}/*`]
  }, (details, callback) => {
    details.requestHeaders['Cookie'] = `curve_hero_session=${token}`
    callback({
      cancel: false,
